...
func (pq *PostQuery) QueryMedias() *MediaQuery {
...
func (pq *PostQuery) WithMedias(opts ...func(*MediaQuery)) *PostQuery {
	query := (&MediaClient{config: pq.config}).Query()
...
//
//	client.Post.Query().
//		GroupBy(post.FieldSubscriptionID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Post.Query().
//		Select(post.FieldSubscriptionID).
//		Scan(ctx, &v)
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withMedias; query != nil {
		if err := pq.loadMedias(ctx, query, nodes,
			func(n *Post) { n.Edges.Medias = []*Media{} },
			func(n *Post, e *Media) { n.Edges.Medias = append(n.Edges.Medias, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
