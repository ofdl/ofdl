...
// The MessageMediaFunc type is an adapter to allow the use of ordinary
// function as MessageMedia mutator.
type MessageMediaFunc func(context.Context, *ent.MessageMediaMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MessageMediaFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MessageMediaMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MessageMediaMutation", m)
}
